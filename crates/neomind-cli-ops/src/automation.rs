...
pub async fn import_automations(client: &ApiClient, data_json: &str) -> Result<CliResponse> {
    let import_data: serde_json::Value =
        serde_json::from_str(data_json).map_err(|e| anyhow::anyhow!("Invalid JSON: {}", e))?;
    let data = client.post("/automations/import", &import_data).await?;
    Ok(CliResponse::success(data, "Automations imported"))
}

/// Enable or disable an automation
pub async fn enable_automation(client: &ApiClient, id: &str, enabled: bool) -> Result<CliResponse> {
    let body = json!({ "enabled": enabled });
    let data = client
        .post(&format!("/automations/{}/enable", id), &body)
        .await?;
...
pub async fn get_automation_executions(client: &ApiClient, id: &str) -> Result<CliResponse> {
    let data = client
        .get(&format!("/automations/{}/executions", id))
        .await?;
    Ok(CliResponse::success(
        data,
