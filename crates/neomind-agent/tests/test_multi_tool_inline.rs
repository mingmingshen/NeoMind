...
async fn test_multi_tools_single_request() -> anyhow::Result<()> {
    ...
    let messages = vec![
        Message::system(
            "You are a helpful assistant. Use tools when needed. When user asks for multiple things, call ALL relevant tools.",
        ),
    ...
    let messages2 = vec![
        Message::system(
            "You are a helpful assistant. Available tools: list_devices, list_rules. Call tools using JSON array format: [{\"name\":\"tool_name\",\"arguments\":{}}]",
        ),
