...
| User asks | Steps |
|-----------|-------|
| Create rule / alert | `device list` → get real metric names → `rule create --dsl 'RULE ... WHEN device.<METRIC> ... END'` → `rule enable <ID>` |
...
- "create rule and enable" → `rule create --dsl '...'` → `rule enable <ID>`
...
- **Rule** (`neomind rule`): Event-triggered conditions. Uses `--dsl 'RULE ... WHEN ... DO ... END'`.
...
- 接入/连接设备 → `neomind system info` + `device-onboarding` skill
