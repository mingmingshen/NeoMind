...
use neomind_core::tools::{Tool, ToolDefinition, ToolError, ToolOutput, ToolCategory, ToolExample, ToolRelationships, Result};
...
impl CreateAutomationTool {
    ...
    async fn generate_dsl_with_llm(&self, description: &str, classification: &IntentClassification) -> Result<String> {
        use neomind_core::{Message, GenerationParams, llm::backend::LlmInput};
...
mod tests {
    ...
    use neomind_core::llm::backend::{LlmError, LlmOutput, FinishReason, TokenUsage, BackendId, StreamChunk};
    use std::pin::Pin;
    ...
    impl LlmRuntime for MockLlm {
        ...
        async fn generate(&self, _input: neomind_core::llm::backend::LlmInput) -> std::result::Result<LlmOutput, LlmError> {
            Ok(LlmOutput {
                text: "OK".to_string(),
        ...
        async fn generate_stream(&self, _input: neomind_core::llm::backend::LlmInput) -> std::result::Result<Pin<Box<dyn futures::Stream<Item = StreamChunk> + Send>>, LlmError> {
