...
use neomind_core::{Message, GenerationParams, llm::backend::{LlmRuntime, LlmInput}};
use neomind_core::tools::Result;
...
impl TaskOrchestrator {
    ...
    pub async fn continue_task(&self, task_id: &str, user_input: &str) -> Result<TaskResponse> {
        ...
        let session = tasks.get_mut(task_id)
            .ok_or_else(|| neomind_core::tools::ToolError::InvalidArguments(format!("Task {} not found", task_id)))?;
        ...
        let current_step = session.steps.get(current_step_idx)
            .ok_or_else(|| neomind_core::tools::ToolError::ExecutionFailed("No more steps".to_string()))?;
