...
impl NativePluginLoader {
    ...
    pub fn load_from_path(&self, path: impl AsRef<Path>) -> Result<LoadedNativePlugin> {
        ...
        let ext = path.extension().and_then(|e| e.to_str()).ok_or_else(|| {
            PluginError::InitializationFailed("Plugin file has no extension".to_string())
        })?;
        ...
        let canonical_path = path.canonicalize().map_err(|e| {
            PluginError::InitializationFailed(format!("Cannot canonicalize path: {}", e))
        })?;
        ...
        let descriptor: Symbol<NativePluginDescriptor> = unsafe {
            library.get(b"neotalk_plugin_descriptor").map_err(|e| {
                PluginError::InitializationFailed(format!("Missing plugin descriptor: {}", e))
            })?
        };
        ...
        // Extract strings
        let id = unsafe { std::slice::from_raw_parts(descriptor.id, descriptor.id_len) };
        let id = String::from_utf8(id.to_vec())
            .map_err(|e| PluginError::InitializationFailed(format!("Invalid plugin ID: {}", e)))?;

        let name = unsafe { std::slice::from_raw_parts(descriptor.name, descriptor.name_len) };
        let name = String::from_utf8(name.to_vec()).map_err(|e| {
            PluginError::InitializationFailed(format!("Invalid plugin name: {}", e))
        })?;

        let version =
            unsafe { std::slice::from_raw_parts(descriptor.version, descriptor.version_len) };
        ...
        let description = String::from_utf8(description.to_vec()).unwrap_or_default();

        let required_version = unsafe {
            std::slice::from_raw_parts(descriptor.required_version, descriptor.required_version_len)
        };
        let required_version =
            String::from_utf8(required_version.to_vec()).unwrap_or_else(|_| "*".to_string());
        ...
        let create_fn: PluginCreateFn = unsafe {
            let sym: Symbol<PluginCreateFn> =
                library.get(b"neotalk_plugin_create").map_err(|e| {
        ...
        let destroy_fn: PluginDestroyFn = unsafe {
            let sym: Symbol<PluginDestroyFn> =
                library.get(b"neotalk_plugin_destroy").map_err(|e| {
...
/// # Usage
/// ```ignore
/// use neomind_core::plugin::native::{export_plugin_descriptor, NEOTALK_PLUGIN_API_VERSION, NativePluginDescriptor};
///
/// struct MyPlugin;
///
