...
///
/// Uses `Arc` for cheap cloning and sharing between tasks.
#[derive(Clone)]
...
impl InFlightRequests {
    ...
    pub fn new(default_timeout: Duration) -> Self {
        Self {
            state: Arc::new(Mutex::new(InFlightState {
                pending: HashMap::new(),
    ...
    pub fn next_request_id(&self) -> RequestId {
        // Get the atomic counter from the state
        // This is safe because we only need atomic ordering, not mutex protection
        // We access the atomic directly through the state
        self.state
            .try_lock()
            .map(|s| s.next_id.fetch_add(1, Ordering::Relaxed))
            .unwrap_or_else(|_| {
                // Fallback: use timestamp-based ID if lock is contested
    ...
    pub async fn complete(&self, request_id: RequestId, response: IpcResponse) -> bool {
        let mut state = self.state.lock().await;
...
mod tests {
    ...
    async fn test_register_and_complete() {
        ...
        let (request_id, rx) = tracker.register().await;

        // Complete the request
        let response = IpcResponse::Pong { timestamp: 123 };
        let found = tracker.complete(request_id, response.clone()).await;
        assert!(found);
    ...
    async fn test_unknown_request() {
        ...
        let response = IpcResponse::Pong { timestamp: 123 };
        let found = tracker.complete(999, response).await;
        assert!(!found);
    }
    ...
    async fn test_concurrent_requests() {
        ...
        let (id1, rx1) = tracker.register().await;
        let (id2, rx2) = tracker.register().await;
        let (id3, rx3) = tracker.register().await;

        assert_eq!(tracker.pending_count().await, 3);

        // Complete them in different order
        tracker
            .complete(id2, IpcResponse::Pong { timestamp: 2 })
            .await;
        tracker
            .complete(id1, IpcResponse::Pong { timestamp: 1 })
            .await;
        tracker
            .complete(id3, IpcResponse::Pong { timestamp: 3 })
            .await;
    ...
    async fn test_clone_and_share() {
        ...
        // Complete with clone
        tracker_clone
            .complete(id1, IpcResponse::Pong { timestamp: 42 })
            .await;
