...
impl Extension for MockEventExtension {
    fn metadata(&self) -> &ExtensionMetadata {
        ...
        META.get_or_init(|| {
            ExtensionMetadata::new("mock.event.extension", "Mock Event Extension", "1.0.0")
        })
    }
...
async fn test_register_in_process_extension() {
    let dispatcher = EventDispatcher::new();
    let ext = Arc::new(tokio::sync::RwLock::new(Box::new(MockEventExtension::new(
        "test.ext",
        vec!["DeviceMetric"],
    )) as Box<dyn Extension>));

    dispatcher
        .register_in_process_extension("test.ext".to_string(), ext)
        .await;
...
fn test_register_isolated_extension_no_subscriptions() {
    ...
    dispatcher.register_isolated_extension("isolated.ext".to_string(), vec![], tx);
...
async fn test_subscription_exact_match() {
    ...
    dispatcher
        .register_in_process_extension("test.ext".to_string(), ext.clone())
        .await;
...
async fn test_subscription_multiple_event_types() {
    ...
    dispatcher
        ...
        .await;
    dispatcher
        .dispatch_event("DeviceAlert", json!({"value": 2}))
        .await;
...
async fn test_mixed_in_process_and_isolated() {
    ...
    dispatcher
        .register_in_process_extension("inprocess.ext".to_string(), ext)
        .await;
