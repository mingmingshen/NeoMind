...
pub enum MockIpcMessage {
    ...
    ExecuteCommand { id: u64, command: String, args: serde_json::Value },
    InitStreamSession { session_id: String, config: serde_json::Value },
    ProcessStreamChunk { session_id: String, sequence: u64, data: Vec<u8> },
    CloseStreamSession { session_id: String },
    Shutdown,
}

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub enum MockIpcResponse {
    Ready { descriptor: MockDescriptor },
    CommandResult { id: u64, result: serde_json::Value },
    StreamSessionInit { session_id: String, success: bool },
    StreamResult { session_id: String, sequence: u64, data: Vec<u8> },
    StreamSessionClosed { session_id: String },
...
impl MockExtensionProcess {
    ...
    pub async fn handle_message(&self, msg: MockIpcMessage) -> MockIpcResponse {
        match msg {
            ...
            MockIpcMessage::ExecuteCommand { id, command, args } => {
                self.command_count.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            ...
            MockIpcMessage::InitStreamSession { session_id, config: _ } => {
            ...
            MockIpcMessage::ProcessStreamChunk { session_id, sequence, data } => {
                let mut sessions = self.sessions.write().await;
    ...
    pub async fn simulate_restart(&self) {
        ...
        self.command_count.store(0, std::sync::atomic::Ordering::SeqCst);
...
mod tests {
    ...
    async fn test_session_lifecycle() {
        ...
        let response = process.handle_message(MockIpcMessage::InitStreamSession {
            session_id: session_id.clone(),
            config: json!({}),
        }).await;
        
        match response {
            MockIpcResponse::StreamSessionInit { session_id: id, success } => {
        ...
        for i in 0..10 {
            let response = process.handle_message(MockIpcMessage::ProcessStreamChunk {
                session_id: session_id.clone(),
                sequence: i,
        ...
        let response = process.handle_message(MockIpcMessage::CloseStreamSession {
            session_id: session_id.clone(),
        }).await;
    ...
    async fn test_process_restart() {
        ...
        for i in 0..3 {
            ...
            process.handle_message(MockIpcMessage::InitStreamSession {
                session_id: session_id.clone(),
                config: json!({}),
            }).await;
        }
    ...
    async fn test_concurrent_requests() {
        ...
        for task_id in 0..num_tasks {
            ...
            let handle = tokio::spawn(async move {
                for req_id in 0..num_requests_per_task {
                    let response = process.handle_message(MockIpcMessage::ExecuteCommand {
        ...
        println!("   ✓ 耗时: {:?}", elapsed);
        println!("   ✓ 吞吐: {:.0} req/sec", 
                 total_requests as f64 / elapsed.as_secs_f64());
        
        // 验证计数
        let count = process.command_count.load(std::sync::atomic::Ordering::SeqCst);
    ...
    async fn test_performance_benchmark() {
        ...
        let start = Instant::now();
        for i in 0..10000 {
            process.handle_message(MockIpcMessage::ProcessStreamChunk {
        ...
        println!("   吞吐: {:.0} chunks/sec", 10000.0 / chunk_time.as_secs_f64());
        
        // 关闭 sessions
        let start = Instant::now();
        for i in 0..1000 {
            process.handle_message(MockIpcMessage::CloseStreamSession {
                session_id: format!("perf-session-{}", i),
            }).await;
        }
