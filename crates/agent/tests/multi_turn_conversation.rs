...
fn create_tool_call_chunks(tool_name: &str, tool_args: serde_json::Value, final_response: &str) -> Vec<StreamChunk> {
    vec![
        ...
        Ok((format!("Based on the tool result, {}", final_response), false)),
    ]
}
...
async fn test_comprehensive_multi_turn_conversation() {
    ...
    let test_rounds: Vec<(&str, /* expected tools */ Vec<&str>, /* chunks */ Vec<StreamChunk>)> = vec![
        // Round 1: Simple greeting (no tools)
        ("Hello", vec![], create_simple_chunks("Hello! How can I help you today?")),

        // Round 2: Query device (uses tool)
        ("What's the status of sensor_1?", vec!["query_device"],
            create_tool_call_chunks("query_device", json!({"device_id": "sensor_1"}),
                "The sensor is online and functioning normally.")),

        // Round 3: List devices (uses tool)
        ("List all devices", vec!["list_devices"],
...
async fn test_repeated_tool_calls_no_loop() {
    ...
    for i in 1..=5 {
        println!("--- Call {} ---", i);
        mock_llm.set_response_chunks(
            create_tool_call_chunks("query_data", json!({"device_id": "sensor_1"}),
                "Data retrieved successfully")
        ).await;

        let result = tokio::time::timeout(
            Duration::from_secs(5),
            session_manager.process_message_events(&session_id, &format!("Query sensor_1 (call {})", i))
        ).await;
...
async fn test_context_preservation() {
    ...
    let conversation = vec![
        ...
        ("What's my name and favorite color?", "Your name is Alice and you like blue."),
    ];

    for (i, (user_msg, expected_contains)) in conversation.iter().enumerate() {
        println!("Turn {}: {}", i + 1, user_msg);
        mock_llm.set_response_chunks(
            create_simple_chunks(expected_contains)
        ).await;
        ...
        match result {
            Ok(Ok(stream)) => {
                let events = collect_events(stream).await;
                let response: String = events.iter()
                    .filter_map(|e| match e {
                ...
                if response.contains(expected_contains) {
                    ...
                    println!("  ❌ Context NOT preserved: got '{}', expected '{}'",
