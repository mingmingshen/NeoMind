...
//!
//! ```rust
//! use edge_ai_agent::tools::{AskUserTool, ConfirmActionTool};
//!
...
use edge_ai_core::tools::ToolCategory;
...
- 不要问过于开放的问题，尽量提供明确的选项"#
    ...
    async fn execute(&self, args: Value) -> Result<ToolOutput, edge_ai_tools::ToolError> {
        ...
        let question = args["question"].as_str().ok_or_else(|| {
            edge_ai_tools::ToolError::InvalidArguments("question must be a string".to_string())
        })?;
...
- 用户确认前不要执行任何实际操作"#
    ...
    async fn execute(&self, args: Value) -> Result<ToolOutput, edge_ai_tools::ToolError> {
        ...
        let action = args["action"].as_str().ok_or_else(|| {
            edge_ai_tools::ToolError::InvalidArguments("action must be a string".to_string())
        })?;
