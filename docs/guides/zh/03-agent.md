...
当 Agent 的 LLM 调用失败时（例如配额用尽、API Key 无效、模型未找到、
网络超时），执行记录将标记为 **失败（Failed）** 并显示真实错误信息。
常见失败场景：

- **HTTP 401/403**：API Key 无效或配额用尽 — 在 LLM 后端设置中修复
  API Key 或充值配额。
- **HTTP 404**：模型名称错误或模型未拉取 — 检查 Agent 的 LLM 后端
  配置的模型名称。
- **HTTP 429**：请求频率受限 — 稍后重试；考虑降低调度频率。
- **HTTP 5xx**：服务商端错误 — 稍后重试。
- **超时**：LLM 响应超过配置的超时时间 — 考虑使用更快的模型或简化
  提示词。

此前，LLM 失败会静默回退到规则分析，产生误导性的"无需操作 - 条件
未满足"输出。本次更新后，LLM 失败将明确显示，便于诊断和修复根本
问题。
